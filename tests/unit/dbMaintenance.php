...
class dbMaintenance
{
    ...
    /**
     * checks which tables of the db changed and then restores this tables
     * for comparison and restoring the dump file '/tmp/tmp_db_dump' is used
     */
                ...
                if ($rs2 && $rs2->RecordCount()>0) {
                    ...
                    while ($aRow = $rs2->fetchRow()) {
                        ...
                        if (!isset($this->_oDBDump[$sTable][$oxid])) {
                            ...
                            } else if ($iMode == MAINTENANCE_WHOLETABLES || $blNoOXID) { //in single table mode tables without oxid are handeld like in whole table mode
    ...
    public function dumpDB()
    {
        ...
        file_put_contents(oxCCTempDir.'/tmp_db_dump_'.$sDbName, $sResult);
     ...
     * @param $aRow a array of the current row in the db
     * @param $sTable the name of the current table
     * @return a sql insert string for the given row
     */
